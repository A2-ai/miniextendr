...
- **Nullable external pointers** — use `ErasedExternalPtr::wrap_sexp` when an R pointer may be null or cleared. `from_sexp` forwards to `from_sexp_unchecked`, which requires a non-null address for its `NonNull` cache; calling it on `new("externalptr")` panics in debug and violates the invariant in release. Raw C fixtures must check this before constructing a handle.
