...
- **Check exported R APIs against the CI floor (R 4.4)** — a declaration in current `Rinternals.h` is not enough. `Rf_charIsUTF8` links on current R but fails package load on R 4.4; use `Rf_getCharCE` for encoding tags, preserving the package-init UTF-8-locale invariant. The Linux R 4.4 package check catches these missing-symbol failures.
