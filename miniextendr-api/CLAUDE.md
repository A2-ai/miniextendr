...
- `deferred_condition.rs` — `defer_warning`/`defer_message`/`defer_condition` (+ `defer_*!` macros): a process-wide `Mutex<Vec<RCondition>>` queue; the generated C wrappers call `mark()` before the body and `finish(mark, value, call)` after it, which signals everything queued in between by `Rf_eval`ing the cached `.miniextendr_raise_condition` closure (built once from `registry::RAISE_CONDITION_HELPER_FN`) inside its own `with_r_unwind_protect`, then returns the value. `with_r_unwind_protect_shim` flushes too. No new list slots or kinds (#1448).
