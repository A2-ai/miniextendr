...
///
/// Called from R via `.Call("miniextendr_write_wrappers", path)` after
/// loading the cdylib with `dyn.load()`. The cdylib includes all symbols
/// (linkme distributed_slice entries, C wrappers, etc.) by design, so
/// no anchor function is needed.
///
/// # Arguments
///
/// * `path_sexp` - STRSXP containing the output file path
///
/// # Returns
///
/// `R_NilValue` on success. Panics (which R catches) on failure.
///
/// # Safety
///
...
pub unsafe extern "C-unwind" fn miniextendr_write_wrappers(
    ...
    unsafe {
        ...
        // Use _unchecked variants: this function is called via cdylib dyn.load(),
        // which doesn't go through R_init_* (so miniextendr_runtime_init() hasn't
        // been called). We're always on R's main thread via .Call().
