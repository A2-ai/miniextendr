...
/// Whether `name` is a syntactic R name (letters, digits, `.` and `_`; starts
/// with a letter or a `.` not followed by a digit; not a reserved word).
///
/// Mirrors `is_syntactic_r_name` in `miniextendr-macros`' `naming` module,
/// which decides how the class generators spell the same names.
#[cfg(not(target_arch = "wasm32"))]
...
/// The argument of a raw `export(...)` NAMESPACE directive: bare for a
/// syntactic name, a double-quoted string for an operator such as `[[`
/// (`export([[)` does not parse). Must match `r_namespace_name` in
/// `miniextendr-macros`' `naming` module, which writes the method blocks'
/// directive for the same generic; roxygen2 only merges identical lines.
#[cfg(not(target_arch = "wasm32"))]
fn r_namespace_name(name: &str) -> String {
...
fn synthesise_generic_doc_block(
    ...
    // An operator generic (`[[`, `%op%`) is not called as `name()`.
    let display = if is_syntactic_r_name(generic) {
        format!("`{generic}()`")
    } else {
        format!("`{generic}`")
    };
    ...
    // Title
    lines.push(format!("#' The {display} generic"));
    ...
    lines.push(format!(
        "#' {display} is {kind_phrase} generated by miniextendr. Methods in this"
    ));
    ...
    if export {
        if kind == "S4" {
            ...
            // including the quoting of operator names).
            lines.push(format!(
                "#' @rawNamespace export({})",
                r_namespace_name(generic)
            ));
        }
    }
...
",
    ...
    let content = wrappers_file_content(&content);
...
///
/// First line of the generated wrappers file.
#[cfg(not(target_arch = "wasm32"))]
const WRAPPERS_FILE_MARKER: &str = "# ---- AUTO-GENERATED FILE - DO NOT EDIT ----";

/// The complete wrappers file for a finished `body`: the marker line, a line
/// naming the `miniextendr-api` version that generated it and an FNV-1a 64
/// digest of the body, then the body (#1552).
///
/// The digest is taken over [`normalize_source_locs`]`(body)`, the same view
/// [`wrappers_semantically_equal`] compares, so a Rust edit that only shifts
/// line numbers leaves the header alone and the no-rewrite check keeps
/// holding; any change to wrapper code, exports or docstrings changes it.
/// It lets a reviewer or a build tool tell two copies of the file apart from
/// the first two lines instead of diffing ~40k lines, and identifies the
/// generator version when a shipped wrapper is reported against.
#[cfg(not(target_arch = "wasm32"))]
fn wrappers_file_content(body: &str) -> String {
    let digest = crate::abi::fnv1a_64(
        normalize_source_locs(body).as_bytes(),
        crate::abi::FNV1A_64_OFFSET,
    );
    format!(
        "{WRAPPERS_FILE_MARKER}\n# miniextendr-api {} | fnv1a-64 of the normalized lines below: {digest:016x}\n{body}",
...
mod tests {
    ...
    // region: wrappers_file_content header (#1552)

    #[test]
    fn wrappers_file_header_carries_version_and_body_digest() {
        // FNV-1a 64 reference vectors: "" and "a".
        assert_eq!(
            crate::abi::fnv1a_64(b"", crate::abi::FNV1A_64_OFFSET),
            0xcbf29ce484222325
        );
        assert_eq!(
            crate::abi::fnv1a_64(b"a", crate::abi::FNV1A_64_OFFSET),
            0xaf63dc4c8601ec8c
        );

        let content = wrappers_file_content("a");
        let mut lines = content.lines();
        assert_eq!(lines.next(), Some(WRAPPERS_FILE_MARKER));
        assert_eq!(
            lines.next(),
            Some(concat!(
                "# miniextendr-api ",
                env!("CARGO_PKG_VERSION"),
                " | fnv1a-64 of the normalized lines below: af63dc4c8601ec8c"
            ))
        );
    ...
    #[test]
    fn wrappers_file_header_ignores_source_position_shifts() {
        let old = "# Generated from Rust fn `foo` (lib.rs:10:5)\nfoo <- function() 1\n";
        let new = "# Generated from Rust fn `foo` (lib.rs:42:8)\nfoo <- function() 1\n";
        let header = |s: &str| wrappers_file_content(s).lines().nth(1).unwrap().to_string();
        assert_eq!(header(old), header(new));
        assert!(wrappers_semantically_equal(
            &wrappers_file_content(old),
            &wrappers_file_content(new)
        ));

        let changed = "# Generated from Rust fn `foo` (lib.rs:10:5)\nfoo <- function() 2\n";
        assert_ne!(header(old), header(changed));
    }
    ...
    #[test]
    fn test_resolve_generic_doc_markers_quotes_operator_exports() {
        for generic in ["[", "[[", "$", "%custom%"] {
            let input = format!(
                ".__MX_GENERIC_DOC__(kind=\"S7\", generic=\"{generic}\", class=\"Foo\", export=true, dispatch=\"x\", no_dots=false)\n"
            );
            let output = resolve_generic_doc_markers(input);
            // Must match the method block's directive (`naming::r_namespace_name`
            // in miniextendr-macros); `export([[)` would not parse.
            assert!(
                output.contains(&format!("#' @rawNamespace export(\"{generic}\")")),
                "{output}"
            );
            assert!(
                output.contains(&format!("#' @name {generic}\n")),
                "{output}"
            );
            assert!(
                output.contains(&format!("#' The `{generic}` generic\n")),
                "{output}"
            );
        }
        let output = resolve_generic_doc_markers(
            r#".__MX_GENERIC_DOC__(kind="S7", generic="get", class="Foo", export=true, dispatch="x", no_dots=false)"#
                .to_string(),
        );
        assert!(
            output.contains("#' @rawNamespace export(get)\n"),
            "{output}"
        );
        assert!(output.contains("#' The `get()` generic\n"), "{output}");
    }

    #[test]
    fn test_is_syntactic_r_name_matches_r() {
        for name in ["get", "var", ".hidden", "s7_trait_Ops_value", "a.b_c1"] {
            assert!(is_syntactic_r_name(name), "{name}");
        }
        for name in [
            "[", "[[", "$", "%custom%", "Foo_[[", "if", "NA", ".1x", "1a", "_a", "",
        ] {
            assert!(!is_syntactic_r_name(name), "{name}");
        }
