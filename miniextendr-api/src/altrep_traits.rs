...
pub trait Altrep {
    ...
    fn length(_x: SEXP) -> R_xlen_t { unreachable!("HAS_LENGTH = false") }
    ...
    fn unserialize(_class: SEXP, _state: SEXP) -> SEXP { unreachable!() }
    ...
    fn duplicate_ex(_x: SEXP, _deep: bool) -> SEXP { unreachable!() }

    const HAS_COERCE: bool = false;
    fn coerce(_x: SEXP, _to_type: SEXPTYPE) -> SEXP { unreachable!() }

    /// Return `Some(true/false)` to override; `None` to keep default.
    const HAS_INSPECT: bool = false;
    fn inspect(_x: SEXP, _pre: i32, _deep: i32, _pvec: i32) -> bool { unreachable!() }
}
...
pub trait AltVec: Altrep {
    ...
    fn extract_subset(_x: SEXP, _indx: SEXP, _call: SEXP) -> SEXP { unreachable!() }
}
...
pub trait AltInteger: AltVec {
    ...
    fn sum(_x: SEXP, _narm: bool) -> SEXP { unreachable!() }
    const HAS_MIN: bool = false;
    fn min(_x: SEXP, _narm: bool) -> SEXP { unreachable!() }
    const HAS_MAX: bool = false;
    fn max(_x: SEXP, _narm: bool) -> SEXP { unreachable!() }
}

/// Real vector methods.
pub trait AltReal: AltVec {
    const HAS_ELT: bool = false;
    fn elt(_x: SEXP, _i: R_xlen_t) -> f64 { unreachable!() }
    const HAS_GET_REGION: bool = false;
    fn get_region(_x: SEXP, _i: R_xlen_t, _n: R_xlen_t, _buf: *mut f64) -> R_xlen_t { unreachable!() }
    const HAS_IS_SORTED: bool = false;
    fn is_sorted(_x: SEXP) -> i32 { unreachable!() }
    const HAS_NO_NA: bool = false;
    fn no_na(_x: SEXP) -> i32 { unreachable!() }
...
pub trait AltRaw: AltVec {
    const HAS_ELT: bool = false;
    fn elt(_x: SEXP, _i: R_xlen_t) -> u8 { unreachable!() }
    const HAS_GET_REGION: bool = false;
    fn get_region(_x: SEXP, _i: R_xlen_t, _n: R_xlen_t, _buf: *mut u8) -> R_xlen_t { unreachable!() }
}

/// String vector methods.
pub trait AltString: AltVec {
    const HAS_ELT: bool = false;
    fn elt(_x: SEXP, _i: R_xlen_t) -> SEXP { unreachable!() }
