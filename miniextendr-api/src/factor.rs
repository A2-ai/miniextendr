...
use crate::ffi::{
    ...
    Rf_isFactor, Rf_mkCharLenCE, Rf_setAttrib, Rf_xlength, SEXP, SEXPTYPE, SexpExt,
    SET_STRING_ELT, STRING_ELT, cetype_t,
};
use crate::from_r::{charsxp_to_str, SexpError};
...
fn factor_class_sexp() -> SEXP {
    *FACTOR_CLASS.get_or_init(|| unsafe {
        ...
        let charsxp =
            Rf_mkCharLenCE(factor_str.as_ptr().cast(), factor_str.len() as i32, cetype_t::CE_UTF8);
...
pub fn build_levels_sexp(levels: &[&str]) -> SEXP {
    unsafe {
        ...
        for (i, level) in levels.iter().enumerate() {
            let charsxp = Rf_mkCharLenCE(level.as_ptr().cast(), level.len() as i32, cetype_t::CE_UTF8);
...
pub fn factor_from_sexp<T: RFactor>(sexp: SEXP) -> Result<T, SexpError> {
    ...
    if idx == NA_INTEGER {
        return Err(SexpError::InvalidValue("NA value in non-Option factor".into()));
    }

    T::from_level_index(idx).ok_or_else(|| {
        SexpError::InvalidValue(format!("factor index {} out of range", idx))
    })
}
...
pub fn factor_vec_from_sexp<T: RFactor>(sexp: SEXP) -> Result<Vec<T>, SexpError> {
    ...
    for i in 0..len {
        ...
        let val = T::from_level_index(idx).ok_or_else(|| {
            SexpError::InvalidValue(format!("factor index {} out of range at position {}", idx, i))
        })?;
        result.push(val);
    }
...
mod tests {
    ...
    fn test_level_index_roundtrip() {
        assert_eq!(TestColor::from_level_index(TestColor::Red.to_level_index()), Some(TestColor::Red));
        assert_eq!(TestColor::from_level_index(TestColor::Green.to_level_index()), Some(TestColor::Green));
        assert_eq!(TestColor::from_level_index(TestColor::Blue.to_level_index()), Some(TestColor::Blue));
    }
