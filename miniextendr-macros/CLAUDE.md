...
- `crate_config.rs` — the only manifest read at expansion time: `[package.metadata.miniextendr]` in the user crate's `Cargo.toml` (via `CARGO_MANIFEST_DIR`, cached per manifest dir). Three keys: `noexport_postfix`, the crate-wide default suffix for `noexport` / `internal` free functions (#1454); `source_tags`, the opt-in for `@source` provenance lines on every generated roxygen block (off by default, #1552; every emitter goes through `roxygen::source_tag`); and `call_attribution = "none" | "wrapper" | "caller"`, the crate-wide default condition-call attribution for standalone fns (#1566; `"caller"` applies to `noexport` / `internal` only). Line-based scanner on purpose (no TOML dependency in a proc-macro crate); add keys there, not ad hoc.
