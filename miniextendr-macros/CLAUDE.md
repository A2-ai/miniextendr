...
- `rust_conversion_builder.rs` (+ dir) — TryFromSexp glue for argument conversion. Every binding is a typed `let` (borrowed slices / `&str` with lifetimes erased via `type_inspect::erase_lifetimes`), because the shared `Err` arm (`conversion_err_arm`) probes the error type by method call (`__mx_conversion_err_parts!`: `RConditionError` class/message/data, else `Display`) and `conversion_condition_value` adds the `failed to convert parameter '<p>' to <T>` context (`type_inspect::type_display`), the crate class and `e$param`.
...
- **Generated `@param` filler** (`(no documentation available)`, `(undocumented)`, choices text, match_arg doc placeholder) is skipped when `roxygen::params_documented_elsewhere` holds for the *author's* tags (`@rdname`, `@describeIn`, `@inheritParams`, `@inherit` with params; #1590). Feed it only doc-comment tags: the framework's own page defaults (write-time file-stem `@rdname`, class-page `@rdname`) must keep their filler, since nothing else documents those arguments. A new per-argument filler emitter goes through the same check.
