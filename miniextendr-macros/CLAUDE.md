...
- `type_inspect.rs` — shared type helpers incl. the visibility-marker peel (`peel_visibility_marker`, `peel_return_visibility`, `resolve_visibility` = marker > `invisible`/`visible` attr > shape default, `visibility_marker_error` for nested / argument-position markers, #1213). Bare fns peel in `lib.rs`; `ParsedMethod` / `TraitMethod` peel at parse time (`visibility_marker` / `invisible` fields) so every return-shape predicate sees the inner type; `MethodReturnBuilder::with_invisible` and `r_wrappers::finish_instance_body` apply the decision.
- `crate_config.rs` — the only manifest read at expansion time: `[package.metadata.miniextendr]` in the user crate's `Cargo.toml` (via `CARGO_MANIFEST_DIR`, cached per manifest dir). Two keys: `noexport_postfix`, the crate-wide default suffix for `noexport` / `internal` free functions (#1454), and `source_tags`, the opt-in for `@source` provenance lines on every generated roxygen block (off by default, #1552; every emitter goes through `roxygen::source_tag`). Line-based scanner on purpose (no TOML dependency in a proc-macro crate); add keys there, not ad hoc.
