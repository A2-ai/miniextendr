...
fn strip_class_system_attrs(mut item_impl: syn::ItemImpl) -> syn::ItemImpl {
    for item in &mut item_impl.items {
        if let syn::ImplItem::Fn(fn_item) = item {
            fn_item
                .attrs
                .retain(|attr| !is_class_system_attr(attr));
        }
    }
    item_impl
}
...
pub fn generate_r6_r_wrapper(parsed_impl: &ParsedImpl) -> String {
    ...
    lines.push(format!("#' @source Generated by miniextendr from Rust type `{}`", type_ident));
    lines.push(format!("#' @field .ptr (private) External pointer to Rust `{}` struct", type_ident));
    lines.push("#' @export".to_string());
...
pub fn generate_s4_r_wrapper(parsed_impl: &ParsedImpl) -> String {
    ...
    // Class definition
    lines.push(format!("#' @source Generated by miniextendr from Rust type `{}`", type_ident));
    lines.push(format!("#' @slot ptr External pointer to Rust `{}` struct", type_ident));
