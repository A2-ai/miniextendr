...
/// Generates full ALTREP registration for a **direct** data struct (no wrapper).
///
/// This is the new single-struct pattern: the data type IS the registration type.
/// Generates TypedExternal, AltrepClass, RegisterAltrep, IntoR, linkme entry, and Ref/Mut.
///
/// Unlike [`generate_altrep_impls`] which assumes a wrapper struct with `self.0`,
/// this generates `IntoR` that wraps `self` directly in an ExternalPtr.
pub(crate) fn generate_direct_altrep_registration(
    ...
    // TYPE_NAME_CSTR and TYPE_ID_CSTR are null-terminated byte strings
    let type_name_bytes = format!("{}\0", type_name_str);
    let type_name_byte_lit = syn::LitByteStr::new(type_name_bytes.as_bytes(), ident.span());
    // TYPE_ID_CSTR uses module_path!() for namespacing
    // (we'll use a concat! expression to get it at compile time)
        ...
        impl ::miniextendr_api::altrep_registration::RegisterAltrep for #ident #ty_generics #where_clause {
            fn get_or_init_class() -> ::miniextendr_api::ffi::altrep::R_altrep_class_t {
                ...
                *CLASS.get_or_init(move || {
                    ...
                    // Install ALTREP methods via InferBase (delegates to bridge installers)
                    unsafe {
