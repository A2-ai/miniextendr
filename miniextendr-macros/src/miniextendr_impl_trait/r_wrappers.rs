...
fn generate_trait_s3_r_wrapper(
    ...
    for method in &instance_methods {
        ...
        let generic_roxygen = generic_roxygen
            ...
            // Explicit target: roxygen2 can't see the generic inside the
            // `if (!exists(...))` guard and would attach a bare `@export` to
            // the next function, exporting the method `generic.Type` as a plain
            // function and leaving the generic unexported (see s3_class.rs).
            .custom(format!("@export {}", generic_name))
            .build();
        ...
        // S3 method roxygen: the method's own @param tags, then a filler for
        // every other formal in its `\usage` (`x` and `...` are documented by
        // the generic block above), like an inherent S3 method's
        // `MethodDocBuilder`: choice text for a `choices(...)` param,
        // `(undocumented)` otherwise. A method whose arguments another block
        // documents (an author topic, `@inheritParams`) gets no filler
        // (#1590). roxygen2 names the block from the S3 method object, so it
        // needs no generated `@name`.
        lines.extend(own_block_page_lines(
            method,
            &type_str,
            None,
...
fn generate_trait_s7_r_wrapper(
    ...
    for method in &instance_methods {
        ...
        if let Some(shortcut_name) = shortcut_name {
            ...
            // filler, which would block the inheritance. A `choices(...)`
            // param's filler is its choice text.
            ...
            for formal in crate::roxygen::split_r_formals(&shortcut_formals) {
                ...
                if pname == "..." {
                    ...
                    let body = choice_docs
                        .get(pname)
                        .map_or("(undocumented)", String::as_str);
                    lines.push(format!("#' @param {pname} {body}"));
                }
            }
            lines.extend(own_block_page_lines(
                method,
...
fn generate_trait_r6_r_wrapper(
    ...
    for method in &instance_methods {
        ...
        // `@R6method Type$method` tag warns "can't find matching R6 method" for
        // anything else), and a trait method is not one. A `@param` here lands
        // in a top-level `\arguments` of the class page, detached from any
        // usage (so `R CMD check` never flags it) and merged by name with every
        // other trait method's parameters, the last block winning.
        lines.extend(own_block_page_lines(
            method,
            &type_str,
            Some(&target),
