...
regenerated by CI (every `R CMD check` run calls [`just vendor`](https://github.com/A2-ai/miniextendr/blob/main/justfile) first) and
...
- `cargo-revendor` runs as a single cargo subcommand, not an R script.
  ...
  [`just revendor-test`](https://github.com/A2-ai/miniextendr/blob/main/justfile), and usable without an R installation.
