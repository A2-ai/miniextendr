...
- **`HashMap`/`BTreeMap` enum DataFrameRow expansion**: `DataFrameRow` derive on map fields emits `unzip()` to produce parallel `_keys` / `_values` vectors in lock-step. Never call `.keys()` then `.values()` separately on the same map binding — use `.into_iter().unzip()` to guarantee the vectors are pairwise aligned (even though Rust's iterator model guarantees stability within a single borrow, `unzip` is explicit and avoids relying on that subtlety).
- **Nested `DataFrameRow` enums**: inner enum must `#[derive(DataFrameRow)]` to satisfy the marker trait for flatten path. `as_factor` opts out of `DataFrameRow` — only `UnitEnumFactor` (auto-emitted by derive for unit-only enums) is needed. The inner enum's `#[dataframe(tag = "variant")]` column appears as `<outer_field>_variant` after prefixing. Split partition names use lowercase snake_case of the variant ident (e.g. `Move` → `move`), while the `_type` column uses the original PascalCase. `as_factor` on a payload-bearing enum is a compile error (missing `UnitEnumFactor`); `as_factor` on `Vec<T>` / slices / scalars is a compile error (`as_factor` requires bare-ident type).
- **Lifetime parameters in `#[miniextendr]`**: `&[T]` and `&str` arguments work without explicit lifetime annotations — the macro converts them internally via `TryFromSexp`. Explicit lifetime params on `#[miniextendr]` functions or impl blocks are rejected (MXL112) because `extern "C-unwind" #[no_mangle]` is incompatible with any generic parameter. Use `Vec<T>` / `String` instead. For `#[derive(DataFrameRow)]` structs, lifetime params on the struct itself propagate correctly (e.g., `struct Foo<'a> { vals: &'a [f64] }`). For DataFrameRow enums, the macro also accepts lifetime params. Borrowed fields (`&'a str`, `&'a [T]`) produce `Vec<Option<&str>>` / `Vec<Option<&[T]>>` companion columns — both have `IntoR` impls as of PR #465 (STRSXP+NA for `&str`, list-column for `&[T]`). Exported `#[miniextendr]` functions cannot carry explicit lifetime params (MXL112); use `'static` literals inside the fn body for fixtures.
