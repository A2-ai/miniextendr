...
- **Concessions and deferred items must become GitHub issues**: whenever a PR doesn't fully solve the problem (scope cut, a known limitation, a latent bug surfaced but not fixed, an approach you accepted knowing it's partial), file a `gh issue create` for each deferred item and reference the issue number in the PR description. Writing "out of scope" or "left for follow-up" in a commit message or PR body without a tracked issue is not acceptable — the reviewer has no way to find it later and no one will own it.
...
- **Prefer `cargo-limit` aliases during iteration**: one-off `cargo` invocations used for the tight edit/compile loop — `cargo check`, `cargo clippy`, `cargo test`, `cargo build` — should be the `cargo-limit` variants (`cargo lcheck`, `cargo lclippy`, `cargo ltest`, `cargo lbuild`). These limit output to the first few errors/warnings so the essential failure surfaces without scrolling through thousands of noise lines. Install once with `just dev-tools-install`. `just`-level recipes and CI keep using plain `cargo` (CI demands full output and `-D warnings`); the limited aliases are for interactive debugging.
