test_that("AltrepSexp detects ALTREP compact integer", {
  x <- 1:10
  expect_true(altrep_sexp_is_altrep(x))
  info <- altrep_sexp_check(x)
...
test_that("AltrepSexp detects ALTREP string (as.character(1:10))", {
  y <- as.character(1:10)
  expect_true(altrep_sexp_is_altrep(y))
  info <- altrep_sexp_check(y)
...
test_that("ensure_materialized works for ALTREP integer", {
  x <- 1:10
  expect_equal(altrep_sexp_ensure_materialized_int(x), 1:10)
})

test_that("ensure_materialized works for non-ALTREP integer", {
  z <- c(1L, 2L, 3L)
  expect_false(altrep_sexp_is_altrep(z))
  expect_equal(altrep_sexp_ensure_materialized_int(z), c(1L, 2L, 3L))
})

test_that("ensure_materialized works for ALTREP strings", {
  y <- as.character(1:10)
  expect_equal(altrep_sexp_ensure_materialized_str(y), as.character(1:10))
})

test_that("ensure_materialized works for non-ALTREP strings", {
  v <- c("a", "b", "c")
  expect_false(altrep_sexp_is_altrep(v))
  expect_equal(altrep_sexp_ensure_materialized_str(v), c("a", "b", "c"))
})

test_that("ensure_materialized preserves NA in strings", {
  v <- c("a", NA, "c")
  result <- altrep_sexp_ensure_materialized_str(v)
...
test_that("non-ALTREP vector correctly detected", {
  expect_false(altrep_sexp_is_altrep(c(1L, 2L, 3L)))
  expect_false(altrep_sexp_is_altrep(c(1.0, 2.0, 3.0)))
  expect_false(altrep_sexp_is_altrep(c("a", "b")))
})
