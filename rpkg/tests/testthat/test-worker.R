...
test_that("panic on worker thread is caught", {
  expect_error(
    rpkg:::unsafe_C_test_worker_panic_simple(),
...
test_that("panic on worker with RAII resources drops them", {
  expect_error(
    rpkg:::unsafe_C_test_worker_panic_with_drops(),
...
test_that("panic inside with_r_thread callback is caught", {
  expect_error(
    rpkg:::unsafe_C_test_worker_panic_in_r_thread(),
...
test_that("panic in with_r_thread with resources drops them", {
  expect_error(
    rpkg:::unsafe_C_test_worker_panic_in_r_thread_with_drops(),
...
test_that("R error inside with_r_thread is propagated", {
  expect_error(
    rpkg:::unsafe_C_test_worker_r_error_in_r_thread(),
    "R error"
  )
})
...
test_that("R error with RAII resources drops worker resources", {
  expect_error(
    rpkg:::unsafe_C_test_worker_r_error_with_drops(),
    "R error"
  )
})
...
test_that("multiple R calls then error propagates error", {
  expect_error(
    rpkg:::unsafe_C_test_worker_r_calls_then_error(),
    "Error"
  )
})
...
test_that("multiple R calls then panic propagates panic", {
  expect_error(
    rpkg:::unsafe_C_test_worker_r_calls_then_panic(),
...
test_that("ExternalPtr can be created with computation on worker", {
  ptr <- rpkg:::unsafe_C_test_extptr_from_worker()
  expect_true(inherits(ptr, "externalptr"))
  expect_equal(rpkg:::unsafe_C_extptr_counter_get(ptr), 100L)
})

test_that("multiple ExternalPtrs can be created from worker context", {
  result <- rpkg:::unsafe_C_test_multiple_extptrs_from_worker()
  expect_true(is.list(result))
  expect_length(result, 2)
  expect_equal(rpkg:::unsafe_C_extptr_counter_get(result[[1]]), 100L)
  expect_equal(rpkg:::unsafe_C_extptr_point_get_x(result[[2]]), 1.5)
  expect_equal(rpkg:::unsafe_C_extptr_point_get_y(result[[2]]), 2.5)
})

# Main thread functions

test_that("main_thread attribute function can call R API", {
  result <- rpkg:::test_main_thread_r_api()
  expect_equal(result, 42L)
})

test_that("main_thread function R error is propagated", {
  expect_error(
    rpkg:::test_main_thread_r_error(),
...
test_that("main_thread function R error with drops still drops resources", {
  expect_error(
    rpkg:::test_main_thread_r_error_with_drops(),
...
test_that("nested with_r_thread with panic drops all resources", {
  expect_error(
    rpkg:::unsafe_C_test_nested_with_panic(),
    "Panic in nested with_r_thread"
  )
})

test_that("deep with_r_thread sequence works", {
  result <- rpkg:::unsafe_C_test_deep_with_r_thread_sequence()
