...
test_that("log::info! outputs to R console", {
  miniextendr:::test_log_set_level("info")
  on.exit(miniextendr:::test_log_set_level("off"), add = TRUE)
  out <- capture.output(miniextendr:::test_log_info("hello from rust"))
  expect_true(any(grepl("hello from rust", out)))
})

test_that("log::warn! outputs to R stderr", {
  miniextendr:::test_log_set_level("warn")
  on.exit(miniextendr:::test_log_set_level("off"), add = TRUE)
  # REprintf goes to stderr — capture via capture.output(type = "message")
  msg <- capture.output(miniextendr:::test_log_warn("watch out"), type = "message")
  expect_true(any(grepl("watch out", msg)))
})

test_that("log::error! outputs to R stderr", {
  miniextendr:::test_log_set_level("error")
  on.exit(miniextendr:::test_log_set_level("off"), add = TRUE)
  msg <- capture.output(miniextendr:::test_log_error("something broke"), type = "message")
...
test_that("debug messages hidden at info level", {
  miniextendr:::test_log_set_level("info")
  on.exit(miniextendr:::test_log_set_level("off"), add = TRUE)
  out <- capture.output(miniextendr:::test_log_debug("hidden debug"))
  expect_false(any(grepl("hidden debug", out)))
})

test_that("debug messages visible at debug level", {
  miniextendr:::test_log_set_level("debug")
  on.exit(miniextendr:::test_log_set_level("off"), add = TRUE)
  out <- capture.output(miniextendr:::test_log_debug("visible debug"))
