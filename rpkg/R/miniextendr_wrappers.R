...
ReceiverCounter$new <- function() {
    self <- .Call(C_ReceiverCounter__new)
...
ReceiverCounter$add <- function() {
    .Call(C_ReceiverCounter__add, self)
}
    ...
    public = list(
        initialize = function(.ptr = NULL) {
            if (!is.null(.ptr)) {
                ...
                private$.ptr <- .Call(C_R6Counter__new)
            }
        },
        value = function() {
        ...
        add = function() {
            .Call(C_R6Counter__add, private$.ptr)
        }
    ),
    private = list(
        .ptr = NULL
...
#' @export
new_s3counter <- function() {
    structure(.Call(C_S3Counter__new), class = "S3Counter")
}
...
s3_add.S3Counter <- function(x, ...) {
    .Call(C_S3Counter__s3_add, x)
}
...
S7Counter <- S7::new_class("S7Counter",
    ...
    constructor = function(.ptr = NULL) {
        if (!is.null(.ptr)) {
            S7::new_object(S7::S7_object(), .ptr = .ptr)
        } else {
            S7::new_object(S7::S7_object(), .ptr = .Call(C_S7Counter__new))
        }
    }
)
...
S7::method(s7_add, S7Counter) <- function(x) .Call(C_S7Counter__s7_add, x@.ptr)
...
#' @export
S4Counter <- function() {
    methods::new("S4Counter", ptr = .Call(C_S4Counter__new))
}
...
#' @exportMethod s4_add
methods::setMethod("s4_add", "S4Counter", function(x) .Call(C_S4Counter__add, x@ptr))
