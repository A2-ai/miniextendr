...
// ---------------------------------------------------------------------------
// fast × error_direct interaction fixture.
//
// With both knobs active, the C wrapper uses `.call = NULL` AND raises
// directly from C (skipping the R-side `.miniextendr_raise_condition`
// re-raise). The condition object is built in C with `call = NULL`, but R's
// `stop()` fills in `sys.call()` for the calling frame when the `call` slot
// is NULL and `call. = TRUE` (the default). As a result `conditionCall(e)`
// is NOT NULL — it carries the call from R's `stop()` attribution.
// ---------------------------------------------------------------------------

/// Identity (i32) with both `fast` and `error_direct` active.
///
/// On the error path: `fast` emits `.call = NULL` so C receives `R_NilValue`
/// for the call SEXP; `error_direct` raises straight from C via
/// `stop(structure(..., call = NULL, ...))`. R's `stop()` fills in the
/// current call when the condition's `call` slot is NULL (the `call.`
/// argument defaults to `TRUE`), so `conditionCall(e)` is still non-NULL.
/// @param x Input value.
/// @export
#[miniextendr(fast, error_direct)]
pub fn fast_and_error_direct(x: i32) -> i32 {
    x
}
