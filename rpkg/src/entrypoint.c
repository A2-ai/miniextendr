...
extern void R_init_miniextendr_miniextendr(DllInfo *dll);
...
// NOTE: R_init function uses miniextendr (case-preserved) to match the
...
void R_init_miniextendr(DllInfo *dll) {
    ...
    // Set ALTREP package name before ALTREP registration
    // Use miniextendr (case-preserved) for R-facing APIs
    miniextendr_set_altrep_pkg_name("miniextendr");
    ...
    R_init_miniextendr_miniextendr(dll);
