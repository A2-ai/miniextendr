...
- `rust-llm-docs/generated/*.md` — LLM-ready Rust API digests and trait-impl
  inventories. **Gitignored**: every public-API change rewrote them, so they
  conflicted on almost every open PR. Regenerate on demand with `just llm-docs`;
  `just llm-docs-check` runs the renderer tests and then regenerates, failing
  on any rustdoc item kind the renderer does not cover.
...
- **Rebase conflicts**: plain `git rebase origin/main`. NEVER `-X theirs` blanket — drops main's changes to shared files (justfile, lockfiles, etc.). Resolve everything by hand **except regenerated artifacts** — never hand-merge their hunks; take either side (`git checkout --theirs` / `git add`) then regenerate:
  ...
  - `rpkg/src/rust/Cargo.lock` pin-only (`#<sha>`) churn → `cargo revendor --manifest-path rpkg/src/rust/Cargo.toml --stamp-lock` after `just configure`.
  - A branch from before `rust-llm-docs/generated/` was untracked hits a modify/delete conflict on those files: `git rm -r --cached rust-llm-docs/generated`, then continue (regenerate locally with `just llm-docs` if you want them).
- **Merge driver for regenerated files (opt-in, once per clone)**: `just merge-drivers-install` defines the `mx-regen` driver (`scripts/merge-driver-regen.sh`) in the repository config and routes the `-merge` paths above to it through `$(git rev-parse --git-path info/attributes)`; both are shared by every worktree. The root `.gitattributes` already routes `rpkg/src/rust/Cargo.lock` to it. With the driver, a both-sides change to one of these files no longer stops the rebase: the driver keeps the **current side** (`%A`, which during a rebase is the branch you rebase onto, i.e. main) and appends the path to `$(git rev-parse --git-dir)/mx-regenerate` (per worktree). Cargo.lock is text-merged first; only a conflict that is pure pin churn resolves to the current side's pins, and any other lock conflict stops as usual with markers. A replayed commit that only touched such files becomes empty and is dropped. After the rebase, run `just regenerate-merged` (it runs the steps above for each listed path, re-stamps the lock last, and clears the list only when every step succeeds), then commit the result. Without the driver installed, git ignores the unknown driver name: Cargo.lock gets the default text merge and the `-merge` paths keep their tracked behaviour. `just test-merge-drivers` (Sync Checks) exercises both cases in scratch repositories.
