...
- `rpkg/src/rust/wasm_registry.rs` (~11.8K lines) — wasm32-target substitute
  for linkme `#[distributed_slice]` tables. Auto-generated by
  `miniextendr_write_wasm_registry` (registered as a C-callable, see
  `miniextendr-api/src/registry.rs`). Regenerated whenever the macro
  surface or wrapper signatures change; committed deliberately so wasm32
  builds work without runtime registration.
- `rpkg/R/miniextendr-wrappers.R` + `NAMESPACE` + `man/*.Rd` — regenerated
  by `just configure && just rcmdinstall && just force-document` after
  macro changes; commit in lockstep (pre-commit hook enforces).
