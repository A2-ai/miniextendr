...
extern void R_init_pigworld_miniextendr(DllInfo *dll);
...
// NOTE: R_init function uses pigworld (case-preserved) to match the
// package name in DESCRIPTION and NAMESPACE useDynLib(). The internal Rust
// module uses pigworld (lowercase with underscores).
void R_init_pigworld(DllInfo *dll) {
    miniextendr_panic_hook();
    miniextendr_worker_init();
    miniextendr_assert_utf8_locale();

    // Set ALTREP package name before ALTREP registration
    // Use pigworld (case-preserved) for R-facing APIs
    miniextendr_set_altrep_pkg_name("pigworld");
    ...
    R_init_pigworld_miniextendr(dll);
