...
# miniextendr-api 0.1.0 | fnv1a-64 of the normalized lines below: 1100ac27c4a454a1
...
SharedData$create <- function(x, y, label) {
  stopifnot(
    "'x' must be double" = is.double(x),
    "'x' must have length 1" = length(x) == 1L,
    "'y' must be double" = is.double(y),
...
EnvPoint$add <- function(dx, dy) {
  stopifnot(
    "'dx' must be double" = is.double(dx),
    "'dx' must have length 1" = length(dx) == 1L,
    "'dy' must be double" = is.double(dy),
...
new_temperature <- function(x) {
  stopifnot("'x' must be double" = is.double(x))
...
producer_self_attributed_probe_impl <- function(x) {
  stopifnot(
    "'x' must be integer" = is.integer(x),
    "'x' must have length 1" = length(x) == 1L
  )
...
SimpleCounter$Counter$add <- function(x, n) {
  stopifnot(
    "'n' must be integer" = is.integer(n),
