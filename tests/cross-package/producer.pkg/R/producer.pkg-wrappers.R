...
#' @title Calculate distance from origin
#' @name s3point_distance.S3Point
#' @rdname S3Point
#' @source Generated by miniextendr from `S3Point::s3point_distance`
...
# EnvPoint::add (105:8)
#' @title Add another point's coordinates
#' @name EnvPoint$add
#' @rdname EnvPoint
...
#' @title Get current value (inherent method)
...
#' @title Get how many history entries exist
...
#' @title Create a `producer_temp` vctrs vector (Celsius temperatures)
#' @description This is the exported constructor consumer.pkg imports via `importFrom(producer.pkg, new_temperature)` to build a producer-owned vctrs type. Dispatch (format/coercion/`vec_c`) then resolves through R's S3 method registry once producer.pkg is loaded.
...
#' @title Create a `producer_oven_temp` vctrs vector that inherits from `producer_temp`
#' @description Exercises `extends` (#1039): the child inherits the parent's format/abbr S3 methods through the class vector, and coercion between the two resolves to the parent (the supertype). consumer.pkg imports this constructor to verify inheritance dispatches across the package boundary.
