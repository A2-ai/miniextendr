...
extern void R_init_consumer_pkg_miniextendr(DllInfo *dll);
...
void R_init_consumer_pkg(DllInfo *dll) {
    miniextendr_panic_hook();
    miniextendr_worker_init();
    miniextendr_assert_utf8_locale();

    // Set ALTREP package name before ALTREP registration
    miniextendr_set_altrep_pkg_name("consumer_pkg");
    ...
    R_init_consumer_pkg_miniextendr(dll);
