...
Direct checkRd validation initially omitted the package-level UTF-8 declaration and reported non-ASCII S7 shortcut prose. Matched the R tools package checker by passing def_enc = TRUE (DESCRIPTION declares UTF-8). S7 constructor generation explicitly copies parameter tags; examples belong on the impl-level class documentation, where ClassDocBuilder emits them. Moved the new example there before regenerating help.
