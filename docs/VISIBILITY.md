...
The same table carries `source_tags = true`, which puts a
`@source Generated by miniextendr from …` line on every generated roxygen
block (free functions, class methods, trait shortcuts, sidecar accessors).
It is off by default (#1552): roxygen2 renders each `@source` as its own
