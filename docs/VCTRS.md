...
The `vctrs DerivedRational;` line registers the generated S3 methods (format, proxy, restore, ptype2, cast).
...
The `vctrs` keyword registers all generated S3 methods for the type. For the impl block approach, use `impl StructName;` instead.
...
- The type is registered with `vctrs StructName;` in `miniextendr_module!`
...
- The type is registered with `vctrs StructName;` in the module
