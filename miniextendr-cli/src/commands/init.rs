...
fn init_package(path: &str, quiet: bool) -> Result<()> {
    ...
    // src/entrypoint.c.in
    write_entrypoint_c_in(root, pkg_name)?;
    ...
    write_document_rs_in(root, pkg_name, &crate_name)?;
...
fn write_entrypoint_c_in(root: &Path, pkg_name: &str) -> Result<()> {
    let pkg_c = pkg_name.replace(['.', '-'], "_");
    let content = format!(
        r#"#include <R.h>
#include <Rinternals.h>
#include <R_ext/Rdynload.h>

void R_init_{pkg_c}(DllInfo *dll) {{
    R_registerRoutines(dll, NULL, NULL, NULL, NULL);
    R_useDynamicSymbols(dll, FALSE);
}}
"#
    );
    let p = root.join("src/entrypoint.c.in");
    if !p.exists() {
        std::fs::write(p, content)?;
    }
    Ok(())
...
fn write_lib_rs(root: &Path, crate_name: &str) -> Result<()> {
    let content = format!(
        r#"use miniextendr_api::{{miniextendr, miniextendr_module}};
...
fn write_document_rs_in(root: &Path, pkg_name: &str, crate_name: &str) -> Result<()> {
    let content = format!(
        r#"//! Document binary — generates R wrapper functions from #[miniextendr] metadata.
//! This file is generated by configure from document.rs.in.

fn main() {{
    let pkg_name = "{pkg_name}";
    let crate_name = "{crate_name}";
    miniextendr_engine::generate_r_wrappers(pkg_name, crate_name);
}}
"#
    );
    let p = root.join("src/rust/document.rs.in");
    if !p.exists() {
        std::fs::write(p, content)?;
    }
    Ok(())
...
# Runs autoconf + configure to generate Makevars, entrypoint.c, etc.
...
fn write_cleanup(root: &Path) -> Result<()> {
    let content =
        "#!/bin/sh\nrm -rf src/rust/target src/Makevars src/entrypoint.c src/rust/.cargo\n";
