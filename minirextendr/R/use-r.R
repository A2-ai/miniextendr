...
#'   subdirectory of a monorepo, where `package.R` lives one level deeper
#'   under the active "monorepo" template type.
#' @return Invisibly returns TRUE if file was created
#' @keywords internal
use_miniextendr_package_doc <- function(path = ".", subdir = NULL) {
  with_project(path)
  data <- template_data()
  ensure_dir(usethis::proj_path("R"))

  save_as <- paste0("R/", data$package, "-package.R")
  use_template("package.R", save_as = save_as, subdir = subdir, data = data)
...
use_miniextendr_namespace <- function(path = ".") {
  ...
  # generated R/<pkg>-package.R doc file. Shared with create_rpkg_subdirectory()
  # (monorepo path) via mx_minimal_namespace().
  namespace_content <- mx_minimal_namespace(pkg_name)
...
use_miniextendr_description <- function(path = ".") {
  ...
  if (!fs::file_exists(license_path)) {
    ...
    writeLines(mx_license_content(pkg_name), license_path)
