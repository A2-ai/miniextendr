...
#'
#' Writes a bindgen wrapper.h that pulls in Rinternals.h (so cli/Matrix/etc.
#' headers see `SEXP`) plus the per-package allowlisted includes.
#'
#' Two output shapes:
...
#'     `--as-cran` (#443).
#'   - When the wrapper lives in a tempdir (dry-run validation from
#'     [check_native_package()]), `r_shim.h` is not on disk next to it,
#'     so include `<Rinternals.h>` directly. Bindgen never sees the shim's
#'     warning-suppression and emits a benign `-Wunknown-warning-option`
